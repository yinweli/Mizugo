...
- procs: 新增 `RavenMessageID`、`RavenErrID` 函式, 從 `*msgs.RavenC` 解析 messageID 與 errID, 輸入非預期型別或 nil 時回傳 0
...
- trials: 新增 `WaitTimeoutCancel` 函式, 支援透過 `context.Context` 取消等待時間
...
- trials: Restore 函式新增重試機制, 解決多個 IDE 同時開啟時因檔案鎖定導致清理失敗的問題
...
- FlagszAND, FlagszOR, FlagszXOR: 內部邏輯優化, 移除對於 `int32` 的強制轉型, 統一使用 `int` 以符合 Go 標準庫規範

## [2.0.8] - 2025-11-28

### Fixed

- 修正 `ZapStream.Caller` 在處理指標接收器時的函式名稱格式化錯誤
