...
import (
	...
	"github.com/yinweli/Mizugo/support/test-client/internal/miscs"
	"github.com/yinweli/Mizugo/support/test-client/internal/modules"
)
...
func (this *Stack) bind(session nets.Sessioner) *nets.Bundle {
	...
	if err := entity.AddModule(modules.NewStack(this.config.Disconnect, this.config.DelayTime)); err != nil {
		wrong = fmt.Errorf("bind: %w", err)
		goto Error
	} // if
