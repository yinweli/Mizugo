...
/// <summary>
/// 客戶端組件範例, 使用Proto訊息處理器, 編碼/解碼流程採用ProcProto, DesCBC以及Base64; 訊息解析採用Raven組件
/// 程式會在Awake時初始化內部組件, 在Start時連線到伺服器, 在Update時更新客戶端組件
/// 連線成功後, 在OnConnect時傳送MProtoRavenQ訊息到伺服器, 等待伺服器的回應
/// 當伺服器回應MProtoRavenA訊息時, 在ProcMProtoRavenA處理它並顯示訊息, 訊息顯示完畢後就斷線
/// 此範例需要配合Mizugo專案的測試伺服器才能正常運作
/// </summary>
public class SampleProtoRaven : MonoBehaviour
{
    private void Awake()
    {
        ...
        client.AddProcess((int)MsgID.ProtoRavenA, ProcMProtoRavenA);
    ...
    /// </summary>
    private void ProcMProtoRavenA(object param)
    {
        var raven = Raven.RavenAParser<HProtoRaven, MProtoRavenQ>(param);
        var errID = (ErrID)raven.errID;
        var duration = stopwatch.ElapsedMilliseconds - raven.request.Time;
        var count = raven.GetRespond<MProtoRavenA>().Count;
    ...
    /// 這裡使用Raven組件來進行這項工作(伺服器也得使用Raven組件)
    /// 這個範例函式專門用來傳送MProtoRavenQ訊息, 訊息中只有一個Time欄位用來填寫當前時間的毫秒
    /// 最後用客戶端組件把訊息傳送出去
    /// </summary>
    private void SendMProtoRavenQ()
    {
        var raven = Raven.RavenQBuilder(
            (int)MsgID.ProtoRavenQ,
            new HProtoRaven() { Token = "protoRaven" },
            new MProtoRavenQ { Time = stopwatch.ElapsedMilliseconds }
        );
    ...
    {
        UnityEngine.Debug.Log("sample proto raven: " + message);
    }
