...
func FlagszSet(input string, index int32, flag bool) string {
	flagMax := int32(len(input)) //nolint:gosec
...
func FlagszAND(input, other string) string {
	...
	for i := 0; i < maxLen; i++ {
		a := FlagszGet(input, int32(i)) //nolint:gosec
		b := FlagszGet(other, int32(i)) //nolint:gosec
...
func FlagszGet(input string, index int32) bool {
	return index < int32(len(input)) && input[index] == FlagszOnRune //nolint:gosec
}
...
func FlagszCount(input string, flag bool) int32 {
	return int32(strings.Count(input, flagsz(flag))) //nolint:gosec
}
