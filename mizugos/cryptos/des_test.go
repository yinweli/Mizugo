...
func (this *SuiteDes) TestDesECB() {
	for size := 16; size <= 64; size++ {
		key := RandDesKey()
		input := []byte(utils.RandString(size, testdata.RandStringLetter))
	...
	keyValid := []byte(utils.RandString(DesKeySize, testdata.RandStringLetter))
	keyInvalid := []byte(utils.RandString(15, testdata.RandStringLetter))
	dataInvalid := []byte(utils.RandString(15, testdata.RandStringLetter))
