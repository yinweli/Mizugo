...
func (this *SuiteCmdRankList) TestRankList() {
	...
	this.meta.table = true
	target = &RankList[testDataRankList]{Meta: &this.meta, Limit: 3, Filter: bson.M{"k2": "2"}, Sort: []SortField{{Field: "k1", Order: -1}}}
	target.Initialize(context.Background(), majorSubmit, minorSubmit)
	this.Nil(target.Prepare())
	_, _ = majorSubmit.Exec(context.Background())
	this.Nil(target.Complete())
	...
	target = &RankList[testDataRankList]{Meta: &this.meta, Limit: 3, Filter: bson.M{}, Sort: []SortField{{Field: "k1", Order: -1}}}
	target.Initialize(context.Background(), majorSubmit, minorSubmit)
	this.Nil(target.Prepare())
	_, _ = majorSubmit.Exec(context.Background())
	this.Nil(target.Complete())
